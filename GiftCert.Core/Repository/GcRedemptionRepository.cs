...
namespace GiftCert.Core.Repository
{
    public class GcRedemptionRepository
    {
        ...
        string url = "http://localhost:8080/api/gcredemption";
        private static List<GcRedemption> gcRedemptions = new List<GcRedemption>();

        public GcRedemptionRepository()
        {
            ...
            if (gcRedemptions != null)
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    try
                    {
                        ...
                        gcRedemptions = JsonConvert.DeserializeObject<List<GcRedemption>>(responseJsonString);
                    }
                    catch (Exception ex)
                    {
        ...
        public List<GcRedemption> GetGcRedemptions()
        {
            return gcRedemptions;
        }

        public GcRedemption GetGcRedemptionById(int orderId)
        {
            IEnumerable<GcRedemption> _gcRedemptions =
                from gcRedemption in gcRedemptions
                where gcRedemption.Remarks == orderId.ToString()
                select gcRedemption;

            return _gcRedemptions.FirstOrDefault();
        }
